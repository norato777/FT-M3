...
Podemos pensar en el procesador (microprocesador) como una pequeña máquina a la que nosotros le pasamos _instrucciones_ que recibe impulsos eléctricos como entrada y genera salidas. Pero el procesador no entiende cualquier lenguaje, sólo habla _lenguaje de máquina_ (es un lenguaje binario, secuencia de unos y ceros) y que está detallado en lo que se conoce como set de instrucciones. Como se imaginarán cada fabricante tiene un set de instrucciones distintos y por lo tanto distintos lenguajes, mencionemos algunos:
...
Hagamos algo más interesante, vamos a `hola.js` y creemos una función y luego la usamos para saludar:
...
Saludar no está definido! recibimos un error. De hecho, está bien que esto suceda. Se acuerdan que dijimos que un módulo no debería afectar otro código accidentalmente? Eso quiere decir que el código está protegido y que no podemos simplemente usarlo y acceder a los objetos fuera de ese módulo.
...
- Eventos del sistema: Estos vienen del código en _C++_, gracias a una librería llamada _libuv_ y manejan eventos que vienen del sistema operativo como por ejemplo: Terminé de leer una archivo, o recibí datos de la red, etc...
